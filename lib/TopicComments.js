...
Meteor.methods({
	createTopicComment : function(topicId, attributes) {
		...
		return TopicComments.createComment(topicId, attributes, user);
	}
})

// Internal routine to create a comment.
TopicComments.createComment = function(topicId, attributes, user) {
	...
	var comment = {
		topicId		: topicId,							// topic we're referencing
	...
	if (commentId) {
		...
		Topics.update(topicId, { $inc : {commentCount: 1} });

		// subscribe the user to the topic
		Topics.subscribeToTopic(user, topicId);

		// Notify any subscribers
		Notifications.notifyTopicComment(user, topicId, commentId, "created");
	}
	return commentId;
}
...
TopicComments.editComment = function(commentId, attributes, user) {
	...
	if (success) {
		// Notify any subscribers
		Notifications.notifyTopicComment(user, topicId, commentId, "modified");
	}
	return success;
}
...
Meteor.methods({
	removeTopicComment : function(commentId) {
		var user = User.getLoggedInUserOrDie();
		TopicComments.removeComment(commentId, user);
	}
});

// Internal routine to remove a comment.
// Makes sure our data structures get updated correctly.
TopicComments.removeComment = function(commentId, attributes, user) {
	// Forget it if the comment can't be found
	var comment = TopicComments.getCommentOrDie(commentId);
	...
	var success = TopicComments.remove(commentId);
	if (success) {
