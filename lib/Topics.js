...
Meteor.methods({
	editTopic : function(topicId, attributes) {
		...
		var otherFields = _.without(Object.keys(attributes), "title", "description", "tags", "reference_url");
...
Topics.editTopic = function(topic, attributes, user) {
	...
	Topics.dieIfTopicExists(attributes, topic);
...
Topics.removeTopic = function (topic) {
	...
	Notifications.markAllReadForTopic(topic);
...
Topics.mergeTopics = function(user, parentTopicId, childTopicId) {
	// Only administrators can do this!
	User.dieIfNotAdmin(user);

	// get the topics in question or error
	var parent = Topics.getTopicOrDie(parentTopicId);
	var child  = Topics.getTopicOrDie(childTopicId);
...
Topics.unsubscribeFromTopic = function(user, topic) {
	...
	if (result) {
		Notifications.markAllReadForTopicAndUser(user, topic);
	}
	return result;
}
