...
// Mark ALL notifications for ALL USERS for topic as read
Notifications.markAllReadForTopic = function(topic) {
	// throw if we can't find the topic
	topic = Topics.getTopicOrDie(topic);
	Notifications.update(
		{type:"topic", itemId:topic._id},
		{$set: {read:true}}
	);
}
...
Notifications.markAllReadForTopicAndUser = function(user, topic) {
	// throw if we can't find the topic or user
	user = User.getUser(user);
	topic = Topics.getTopicOrDie(topic);
	Notifications.update(
		{type:"topic", itemId:topic._id, userId:user._id},
		{$set: {read:true}}
	);
}
...
Notifications.notifyTopicCreated = function(changer, topic) {
	...
	Notifications._notifyAdministrators(topic._id, message, route, changer._id);
}
...
Notifications.notifyTopicChanged = function(changer, topic) {
	...
	Notifications._notifyTopicSubscribers(topic._id, message, route, changer._id);
}
