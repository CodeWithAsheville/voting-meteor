...
// Throw an exception if the user is not an administrator.
User.dieIfNotAdmin = function(user) {
	if (!User.isAdmin) {
		throw new Meteor.Error("authentication-not-admin", "You must be an administrator to do this.");
	}
	return true;
}

// Return an array of USERIDs of all of the admin users.
User.getAllAdmins = function() {
	var admins = Meteor.users.find({ roles : {$in : ["admin"] } }).fetch();
	return _.pluck(admins, "_id");
}
